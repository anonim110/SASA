...
bot.action(/mint(\d+)/, async (ctx) => {
  ...
  const tonLink =
    `ton://transfer/${WALLET_TON}?amount=${Math.floor(amount * 1e9)}&text=${encodeURIComponent(comment)}`;
...
bot.action(/check_/, async (ctx) => {
  ...
  await ctx.editMessageText(
    `Платёж не найден\n\nПроверь сумму и комментарий`,
    {
      reply_markup: {
        inline_keyboard: [
          [{ text: 'Проверить снова', callback_data: ctx.callbackQuery.data }]
        ]
      }
    }
  );
